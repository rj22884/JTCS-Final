"""Signed SSO token so ERP admins open website recruitment without a second login."""
...
def recruitment_public_base() -> str:
    explicit = (current_app.config.get("RECRUITMENT_PUBLIC_URL") or "").strip().rstrip("/")
    if explicit:
        return explicit
...
def sso_secret() -> str:
    return (current_app.config.get("RECRUITMENT_SSO_SECRET") or DEFAULT_SSO_SECRET).strip()
