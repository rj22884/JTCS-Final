...
class PdsMasterRepository:
    ...
    def list_rows(self, model, *, search: str | None = None, filters: dict | None = None, limit: int = 2000):
        self.ensure_schema()
        stmt = select(model)
        filters = filters or {}
        ...
        name_col = next(
            (getattr(model, name) for name in ("StateName", "DistrictName", "DsoName", "AroName", "SiName", "FpsName", "RcNumber") if hasattr(model, name)),
            pk,
        )
        stmt = stmt.order_by(name_col, pk).limit(limit)
